...
typedef struct PhantomCiphertext {
    ...
    inline void load(const PhantomContext &context, std::istream &stream) {
        ...
        try {
            ...
            new_data.data_.acquire(
                phantom::util::allocate<uint64_t>(phantom::util::global_pool(), total_uint64_count));
     ...
     */
    void resize(const PhantomContext &context, size_t chain_index, size_t size, bool fill_zero = false) {
        ...
        if (new_size > old_size) {
            ...
            if (fill_zero)
                CUDA_CHECK(cudaMemset(data_.get() + old_size, 0, (new_size - old_size) * sizeof(uint64_t)));
        }
