...
namespace phantom::util {
    ...
    void MemoryPoolHeadST::release() noexcept {
        MemoryPoolItem *old_first = first_item_;
        while (old_first) {
            MemoryPoolItem *next_item = old_first->next();
            cudaFree(old_first->data());
            delete old_first;
            old_first = next_item;
        }
    }
    ...
    void MemoryPoolHeadMT::release() noexcept {
      bool expected = false;
