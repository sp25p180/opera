...
void PhantomContext::set_galois_elts(const std::vector<uint32_t> &galois_elts) {
    int log_n = phantom::util::get_power_of_two(poly_degree_);
    bool is_bfv = key_galois_tool_->is_bfv();
    key_galois_tool_ = std::make_shared<PhantomGaloisTool>(galois_elts, log_n, is_bfv);
}